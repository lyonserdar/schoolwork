...
class EasyFrame(Tkinter.Frame):
    ...
    def __init__(
        self, title="", width=None, height=None, background="white", resizable=False
    ):
    ...
    def addLabel(
        ...
        label = Tkinter.Label(
            self, text=text, font=font, background=background, foreground=foreground
        )
    ...
    def addButton(
        ...
        button = Tkinter.Button(self, text=text, command=command, state=state)
        self.rowconfigure(row, weight=1)
        self.columnconfigure(column, weight=1)
        button.grid(
            row=row,
    ...
    def addTextArea(
        self, text, row, column, rowspan=1, columnspan=1, width=80, height=5, wrap=NONE
    ):
        ...
        area = TextArea(frame, text, width, height, xScroll.set, yScroll.set, wrap)
        area.grid(row=0, column=0, padx=5, pady=5, sticky=N + S + E + W)
    ...
    def addCanvas(
        ...
        if not canvas:
            canvas = EasyCanvas(self, width=width, height=height, background=background)
        canvas.grid(
            row=row,
            column=column,
            rowspan=rowspan,
    ...
    def addMenuBar(self, row, column, rowspan=1, columnspan=1, orient="horizontal"):
        ...
        menuBar.grid(
            row=row, column=column, rowspan=rowspan, columnspan=columnspan, sticky=N + W
        )
        return menuBar
    ...
    def addRadiobuttonGroup(
        self, row, column, rowspan=1, columnspan=1, orient=VERTICAL
    ):
        """Creates and returns a radio button group."""
        return EasyRadiobuttonGroup(self, row, column, rowspan, columnspan, orient)

    # Added 12-18-2012
    def addPanel(self, row, column, rowspan=1, columnspan=1, background="white"):
...
class AbstractField(Tkinter.Entry):
    ...
    def __init__(self, parent, value, width, state):
        ...
        Tkinter.Entry.__init__(
            self, parent, textvariable=self.var, width=width, state=state
        )
...
class TextArea(Tkinter.Text):
    ...
    def __init__(
        self, parent, text, width, height, xscrollcommand, yscrollcommand, wrap
    ):
...
class EasyRadiobuttonGroup(Tkinter.Frame):
    ...
    def addRadiobutton(self, text, command=lambda: 0):
        ...
        button = Tkinter.Radiobutton(
            self, text=text, value=text, command=command, variable=self._commonVar
        )
...
class EasyCheckbutton(Tkinter.Checkbutton):
    ...
    def __init__(self, parent, text, command):
        ...
        Tkinter.Checkbutton.__init__(
            self, parent, text=text, variable=self._variable, command=command
        )
...
class EasyMenubutton(Tkinter.Menubutton):
    ...
    def __init__(self, menuBar, text, state):
        Tkinter.Menubutton.__init__(self, menuBar, text=text, state=state)
...
class EasyCanvas(Tkinter.Canvas):
    ...
    def __init__(self, parent, width=None, height=None, background="white"):
        Tkinter.Canvas.__init__(
            self, parent, width=width, height=height, background=background
        )
    ...
    def drawLine(self, x0, y0, x1, y1, fill="black", width=1):
        item = self.create_line(x0, y0, x1, y1)
        self.itemconfig(item, fill=fill, width=width)
        return item

    def drawRectangle(self, x0, y0, x1, y1, outline="black", fill=None):
    ...
    def drawOval(self, x0, y0, x1, y1, outline="black", fill=None):
    ...
    def drawImage(self, image, x, y, anchor=CENTER):
        ...
        item = self.create_image(x, y, image=image, anchor=anchor)
...
class EasyDialog(tkSimpleDialog.Dialog):
    ...
    def addButton(
        ...
        """Creates and inserts a button at the row and column,
        and returns the button."""
        button = Tkinter.Button(master, text=text, command=command, state=state)
        master.rowconfigure(row, weight=1)
        master.columnconfigure(column, weight=1)
        button.grid(
            row=row,
    ...
    def addRadiobuttonGroup(
        self, master, row, column, rowspan=1, columnspan=1, orient=VERTICAL
    ):
        """Creates and returns a radio button group."""
    ...
    def addMenuBar(
        self, master, row, column, rowspan=1, columnspan=1, orient="horizontal"
    ):
        """Creates and inserts a menu bar at the row and column,
        and returns the menu bar."""
        if not orient in ("horizontal", "vertical"):
    ...
    def addPanel(
        self, master, row, column, rowspan=1, columnspan=1, background="white"
    ):
        """Creates and returns a panel."""
